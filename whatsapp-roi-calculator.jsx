...
function ChannelInputPanel({channel,inputs,onChange,region,country,rateLabel}){
  ...
  return(<Card style={{borderTop:`3px solid ${cfg.color}`,marginBottom:16}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}><Icon name={cfg.icon} size={20} color={cfg.color}/><span style={{fontFamily:T.fontDisplay,fontWeight:800,fontSize:18,color:cfg.dark}}>{cfg.label}</span><span style={{padding:"3px 10px",borderRadius:12,background:cfg.bg,fontSize:11,fontFamily:T.fontMono,color:cfg.dark,fontWeight:500}}>{cont}</span></div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:"0 20px"}}>
      ...
      <InputField label="Broadcasts / Month" value={inputs.broadcastsPerMonth||4} onChange={v=>u("broadcastsPerMonth",v)} tooltip="How many times per month do you send broadcast campaigns? e.g. 4 = weekly, 8 = twice a week" min={1} max={30}/>
...
export default function App(){
  ...
  return(<div style={{minHeight:"100vh",background:T.appBg,padding:"0 16px 40px"}}><style>{globalCSS}</style>
    ...
    <div style={{maxWidth:960,margin:"0 auto",paddingTop:24,animation:"fadeUp 0.5s ease both"}}>
      ...
      {showSc&&<Card style={{marginBottom:16}}><div style={{fontSize:14,fontWeight:700,fontFamily:T.fontDisplay,marginBottom:12,display:"flex",alignItems:"center",gap:8}}><Icon name="zap" size={14}/> Quick-Start Scenarios</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:8}}>{SCENARIOS.map((sc,i)=><button key={i} onClick={()=>applySc(sc)} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",borderRadius:T.radiusXs,border:`1px solid ${T.border}`,background:T.surfaceLight,cursor:"pointer",textAlign:"left"}}><Icon name={sc.icon} size={22} color={T.textMuted}/><div><div style={{fontSize:13,fontWeight:600,fontFamily:T.font,color:T.text}}>{sc.name}</div><div style={{fontSize:11,color:T.textMuted}}>{sc.country} &middot; ${sc.dealValue}</div></div></button>)}</div></Card>}
      ...
      {/* STEP 1: Industry */}
      {step===1&&<Card delay={0.05}><h2 style={{fontFamily:T.fontDisplay,fontWeight:800,fontSize:20,marginBottom:4}}>Select Your Industry</h2><p style={{fontSize:13,color:T.textMuted,marginBottom:20}}>Industry benchmarks determine default conversion rates</p>{["direct","leadgen","footfall"].map(arch=>{const al=arch==="direct"?"Direct Commerce":arch==="leadgen"?"Lead Generation":"Footfall";return <div key={arch} style={{marginBottom:20}}><div style={{fontSize:12,fontWeight:700,color:T.textMuted,fontFamily:T.fontSora,marginBottom:10}}>{al}</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:8}}>{INDUSTRIES.filter(i=>i.archetype===arch).map(ind=>{const cv=region?CONV[region]?.[ind.name]:null,sel=industry===ind.name;return <button key={ind.name} onClick={()=>setIndustry(ind.name)} style={{padding:"14px 18px",borderRadius:T.radiusSm,textAlign:"left",border:sel?`2px solid ${T.green}`:`1px solid ${T.border}`,background:sel?T.lightGreenBg:T.surface,cursor:"pointer",transition:"all 0.15s ease"}}><div style={{fontFamily:T.fontDisplay,fontWeight:700,fontSize:14,color:sel?T.darkGreen:T.text,marginBottom:4}}>{ind.name}</div><div style={{fontSize:11,color:T.textMuted}}>{ind.rateLabel}{cv&&<span style={{marginLeft:8,fontFamily:T.fontMono}}>~{cv}%</span>}</div></button>})}</div></div>})}</Card>}
